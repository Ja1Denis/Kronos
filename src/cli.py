...
def stats():
    ...
    try:
        ...
        # Model breakdown
        if l_stats.get('model_breakdown'):
            breakdown_lines = []
            for model, data in l_stats['model_breakdown'].items():
                breakdown_lines.append(f"• {model}: {data['tokens']:,} t (${data['usd']:.2f})")
            table.add_row(STRINGS.LABEL_MODEL_SAVINGS, "\n".join(breakdown_lines))
